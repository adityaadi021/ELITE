...
class Announce(commands.Cog):
    ...
    async def dmuser(self, ctx, user: discord.User = None, message: str = None, attachment: discord.Attachment = None):
        if not await is_admin(ctx):
            if isinstance(ctx, discord.Interaction):
                await ctx.response.send_message("❌ You do not have permission to use this command.", ephemeral=True)
            else:
        ...
        if isinstance(ctx, discord.Interaction):
            # Slash command - use parameters
            if not message and not attachment:
                await ctx.response.send_message("❌ You must provide at least text or an attachment.", ephemeral=True)
                return
            embed = modern_embed(
            ...
            embed.set_footer(text=f"From {ctx.guild.name} • Requested by {ctx.user}", icon_url=ctx.user.avatar.url if ctx.user.avatar else None)
            try:
                await user.send(embed=embed)
                await ctx.response.send_message(f"✅ DM sent to {user.mention}", ephemeral=True)
            except Exception:
                await ctx.response.send_message(f"❌ Could not DM user.", ephemeral=True)
        else:
            # Prefix command - use interactive prompt
